...
The modules :mod:`iminuit.errordef.ml` (for likelihood functions) and :mod:`iminuit.errordef.lsq` (for least-squares functions a.k.a. :math:`\chi^2` functions) contain special constants ``sigma`` and ``cl`` to conveniently set the error definition in the :class:`Minuit` class. These constants are not normal numbers, they are instances of the classes :class:`iminuit.util.ErrorDefSigma` and :class:`iminuit.util.ErrorDefCL`, respectively, which have overloaded operators so that the syntax becomes straight forward. How to use these is best explained with some examples.
